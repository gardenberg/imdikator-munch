...
#27. juli 2016
library(tidyr)
barn <- read.csv("~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/tab30_bydel_1_2015.sdv", row.names=NULL, stringsAsFactors=FALSE)
...
library(dplyr)
#bydel_1
barn <- read.csv("~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/barnehagedeltakelse_spraak-bydel-2015.csv", row.names=NULL, stringsAsFactors=FALSE,colClasses="character")
#er alle variabler her?
nrow(barn)==18*6*3*2
#hvis ikke, muter inntil du finner mangler
isna = sum(is.na(barn)==T)
df = spread(barn,enhet,tabellvariabel,drop=FALSE)
df = gather(df,enhet,tabellvariabel,6:7)
#er variabler kodet riktig?
apply(subset(barn,select=-tabellvariabel),2, function(x){levels(as.factor(x))})
apply(barn,2, function(x){nlevels(as.factor(x))})
#substansielt, ser det rimelig ut?
t = filter(barn,bydel_=="030101")

#bydel_2
barn <- read.csv("~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/barnehagedeltakelse_barnikommune-bydel-2015.csv", row.names=NULL, stringsAsFactors=FALSE,colClasses="character")
#er alle variabler her?
nrow(barn)==18*6*3*1
#hvis ikke, muter inntil du finner mangler
isna = sum(is.na(barn)==T)
...
df = gather(df,enhet,tabellvariabel,6:7)
#beregner prosenter
df = spread(barn,barnikommune,tabellvariabel,drop=FALSE)
names(df)[6:8]=paste0(names(df)[6:8],"-personer")
df$`alle-prosent`=extract_numeric(df$`alle-personer`)/extract_numeric(df$`alle-personer`)*100
df$`barn_ellers-prosent`=extract_numeric(df$`barn_ellers-personer`)/extract_numeric(df$`alle-personer`)*100
df$`innvandrerbakgr-prosent`=extract_numeric(df$`innvandrerbakgr-personer`)/extract_numeric(df$`alle-personer`)*100
df = gather(df,barnikommune,tabellvariabel,6:11)
df = select(df,-enhet)
df = separate(df,barnikommune,into=c("barnikommune","enhet"),sep="-")
#na-verdier må kodes som manglende
...
apply(barn,2, function(x){nlevels(as.factor(x))})
#substansielt, ser det rimelig ut?
t = filter(barn,bydel_nr=="01")
#skriver ut fila med prosenter
write.csv(df,"~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/beregna prosent/barnehagedeltakelse_barnikommune-bydel-2015.csv",row.names=F)

#kommune_1
barn <- read.csv("~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/barnehagedeltakelse_spraak-kommune-2015.csv", row.names=NULL, stringsAsFactors=FALSE,colClasses="character")
#er alle variabler her?
nrow(barn)==429*6*3*2
#hvis ikke, muter inntil du finner mangler
isna = sum(is.na(barn)==T)
...
apply(barn,2, function(x){nlevels(as.factor(x))})
#substansielt, ser det rimelig ut?
t = filter(barn,kommune_nr=="0101")

#kommune_2
barn <- read.csv("~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/barnehagedeltakelse_barnikommune-kommune-2015.csv", row.names=NULL, stringsAsFactors=FALSE,colClasses="character")
#er alle variabler her?
nrow(barn)==429*6*3*1
#hvis ikke, muter inntil du finner mangler
isna = sum(is.na(barn)==T)
...
#substansielt, ser det rimelig ut?
t = filter(barn,kommune_nr=="0101")
write.csv(df,"~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/beregna prosent/barnehagedeltakelse_barnikommune-kommune-2015.csv",row.names=F)

#fylke_1
barn <- read.csv("~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/barnehagedeltakelse_spraak-fylke-2015.csv", row.names=NULL, stringsAsFactors=FALSE,colClasses="character")
#er alle variabler her?
nrow(barn)==20*6*3*2
#hvis ikke, muter inntil du finner mangler
isna = sum(is.na(barn)==T)
df = spread(barn,enhet,tabellvariabel,drop=FALSE)
df = gather(df,enhet,tabellvariabel,6:7)
#mangler enhet=prosent. det går fint.
#er variabler kodet riktig?
apply(subset(barn,select=-tabellvariabel),2, function(x){levels(as.factor(x))})
apply(barn,2, function(x){nlevels(as.factor(x))})
#substansielt, ser det rimelig ut?
t = filter(barn,fylke_nr=="00")

#fylke_2
barn <- read.csv("~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/barnehagedeltakelse_barnikommune-fylke-2015.csv", row.names=NULL, stringsAsFactors=FALSE,colClasses="character")
#er alle variabler her?
nrow(barn)==20*6*3*1
#hvis ikke, muter inntil du finner mangler
isna = sum(is.na(barn)==T)
...
#substansielt, ser det rimelig ut?
t = filter(barn,fylke_nr=="00")
write.csv(df,"~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/beregna prosent/barnehagedeltakelse_barnikommune-fylke-2015.csv",row.names=F)

#naringsregion_2
barn <- read.csv("~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/barnehagedeltakelse_barnikommune-naringsregion-2015.csv", row.names=NULL, stringsAsFactors=FALSE,colClasses="character")
#er alle variabler her?
nrow(barn)==84*6*3*2
#hvis ikke, muter inntil du finner mangler
isna = sum(is.na(barn)==T)
...
apply(barn,2, function(x){nlevels(as.factor(x))})
#substansielt, ser det rimelig ut?
t = filter(barn,naringsregion_nr=="01")
write.csv(df,"~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/beregna prosent/barnehagedeltakelse_barnikommune-naringsregion-2015.csv",row.names=F)

#naringsregion_1
barn <- read.csv("~/Indikatorprosjektet/Indikatorer og datagrunnlag/Dataleveranser/Leveranse 1-2016 fra SSB/30 - barnehagedeltakelse/barnehagedeltakelse_spraak-naringsregion-2015.csv", row.names=NULL, stringsAsFactors=FALSE,colClasses="character")
#er alle variabler her?
nrow(barn)==84*6*3*2
#hvis ikke, muter inntil du finner mangler
isna = sum(is.na(barn)==T)
...
tilskudd <- read.csv("~/R/imdikator-munch/data_flat_input/norsktilskudd-kommune-2011_2012.csv", row.names=NULL, na.strings="NA", stringsAsFactors=FALSE, sep=";", dec=",")
tilskudd = subset(tilskudd,select=-kommune_navn)
df = spread(tilskudd,aar,tabellvariabel,drop=F)
sum(df$`2011`,na.rm=T)

#NORSKTILSKUDD - KOMMUNE 2011_2012
#2. august 2016
library(tidyr)
library(dplyr)
tilskudd <- read.csv("~/R/imdikator-munch/data_flat_input/norsktilskudd-kommune-2011_2012.csv", row.names=NULL, na.strings="NA", stringsAsFactors=FALSE, sep=";", dec=",",colClasses="character")
#1. kommune_navn fjernes
tilskudd = subset(tilskudd,select=-kommune_navn)
#er det riktig antall nivåer og riktig koding?
apply(subset(tilskudd,select=-tabellvariabel),2, function(x){levels(as.factor(x))})
apply(tilskudd,2, function(x){nlevels(as.factor(x))})
#inderøy skifta fra 1729 til 1756. Oslo er ført opp som 0300
df = spread(tilskudd,aar,tabellvariabel,drop=F)


subset(df,kommune_nr=="1729")
subset(df,kommune_nr=="1756")
#disse to står med sine respektive kommunenummer i de respektive åra
subset(df,kommune_nr=="0300"|kommune_nr=="0301")
#må kodes om, 0300 er aldri brukt andre steder
df$kommune_nr[df$kommune_nr=="0300"]="0301"
subset(df,kommune_nr=="0300"|kommune_nr=="0301")
#mangler en del andre kommuner
kinfo <- read.csv("~/R/imdikator-munch/parameters/kommuneegenskaper.csv", sep=",", stringsAsFactors=FALSE,colClasses="character")
#kinfo = select(kinfo,kommune_nr,standard_utgave)
#kinfo = filter(kinfo,standard_utgave=="2008"|standard_utgave=="2012")
df = merge(df,subset(kinfo,standard_utgave=="2008",select="kommune_nr"),by="kommune_nr",all.x=T,all.y=T)
df = merge(df,subset(kinfo,standard_utgave=="2012",select="kommune_nr"),by="kommune_nr",all.x=T,all.y=T)
df = gather(df,aar,tabellvariabel,5:6)
#alle kommuner som er lagt til må få 0 som tabellvariabel. Det er ingen NA, bare 0
#selv om strengt tatt så eksisterte jo ikke alle disse kommunene i begge år.
sum(is.na(df$tabellvariabel))
df$tabellvariabel[is.na(df$tabellvariabel)==T] = 0
sum(is.na(df$tabellvariabel))
sum(df$tabellvariabel==".")
sum(df$tabellvariabel==":")
#alle kommuner som er lagt til må også få enhet, tabell_navn og tilskudd_til_kommuner
#siden disse variablene kun tar en verdi kan disse settes her
df$enhet="kroner"
df$tabell_navn="tilskudd"
df$tilskudd_til_kommuner="norsk_samf_tilsk"
#sjekk av at spread kommer ikke tilbake med duplicate identifiers.
t = spread(df,tilskudd_til_kommuner,tabellvariabel,drop=F)
norsktilskudd = df

#merger tilskuddsdataene til allerede eksisterende tilskuddsdata
#tilnærming: merge-overwrite
tilskudd <- read.csv("~/R/imdikator-munch/data_flat_input/tilskudd-alle-2011_2014.csv", na.strings="", stringsAsFactors=FALSE,fileEncoding = "UTF-8-BOM",colClasses="character")
tilskudd_kommune = subset(tilskudd,kommune_nr!="NULL",select=-c(fylke_nr,naringsregion_nr))
#bretter de ut
df_t = spread(tilskudd_kommune,tilskudd_til_kommuner,tabellvariabel)
df_n = spread(subset(norsktilskudd,select=c("kommune_nr","aar","tilskudd_til_kommuner","tabellvariabel")),tilskudd_til_kommuner,tabellvariabel)
t = merge(df_t,df_n,by=c("kommune_nr","aar"),all.x=T,all.y=T)
#t$norsk_samf_tilsk.x[t$norsk_samf_tilsk.x=="."] # = 0
#t$norsk_samf_tilsk.y[is.na(t$norsk_samf_tilsk.y)==T] #= 0
t$norsk_samf_tilsk[is.na(t$norsk_samf_tilsk.y)==F]=t$norsk_samf_tilsk.y[is.na(t$norsk_samf_tilsk.y)==F]
t$norsk_samf_tilsk[is.na(t$norsk_samf_tilsk.y)==T]=t$norsk_samf_tilsk.x[is.na(t$norsk_samf_tilsk.y)==T]
t=subset(t,select=-c(norsk_samf_tilsk.y,norsk_samf_tilsk.x))
apply(t,2, function(x){nlevels(as.factor(x))})
#så må jeg beregne nye totaler for gruppen det gjelder
t$totalt_2 =extract_numeric(t$barnehage_tilsk)+extract_numeric(t$eldre_tilsk)+extract_numeric(t$em_tilsk)+extract_numeric(t$funkhem)+extract_numeric(t$inttilsk_1_5)+extract_numeric(t$norsk_samf_tilsk)+extract_numeric(t$skole_tilsk)
t$totalt= apply(select(t,barnehage_tilsk:inttilsk_1,inttilsk_2_5:skole_tilsk,norsk_samf_tilsk),1,
      function(x){
              sum(extract_numeric(x),na.rm=T)
      }
)
sum(t$totalt_2==t$totalt_3,na.rm=T)==nrow(t,na.rm=T)

#sjekker for na og andre feil
apply(t,2, function(x){sum(is.na(x))})
naset <- subset(t,subset=(is.na(tabell_navn)==T))
#NA på enhet, tabellnavn kan løses 
t$enhet="kroner"
t$tabell_navn="tilskudd"
#NA på kommune 1723, 1756, 1903, 1915 og 9999. 
#1915 kan beholdes, dette er en kommune som ikke fikk andre tilskudd og som opphørte 1.1.2013
#1723, 1756, 1903 fantes ikke (i hele perioden) og kan fjernes. 
#men fjerning fører til uheldige problemer lenger ned enn så lenge, bør derfor heller nulles, som praksis lenger opp.
#gjøres best på gathered fil.
df=gather(select(t,-totalt_2,-totalt_3,-totalt_4),tilskudd_til_kommuner,tabellvariabel,barnehage_tilsk:norsk_samf_tilsk)
sum(is.na(tilskudd_kommune$tabellvariabel))
sum(is.na(df$tabellvariabel))
#legger også til andre år som pådrar seg missing-verdier.
t = spread(df,aar,tabellvariabel,drop=F)
t2 = gather(t,aar,tabellvariabel,5:8)
naset= subset(t2,is.na(tabellvariabel)==T)
t2$tabellvariabel[is.na(t2$tabellvariabel)==T]="0"
#9999 bør ikke brukes.
tilskudd_ut = filter(t2,kommune_nr!="9999")

#eksporterer fil
write.csv(tilskudd_ut,"data_flat_output/tilskudd-kommune-2011_2014.csv",row.names=F)


#AGGREGERING
#har valget mellom å aggregere i samme datasett, eller skjære det opp i tre biter.
#virker enklere å skjære det opp, for da kan jeg gjenbruke koden for aggregering fra under.
#...hvis da det lar seg aggregere på tvers av tid.

#AGGREGERING TILSKUDD 2015
#26. januar, 3. februar, 4. august 2016

#næringsregion 2011-2014
library(reshape)
library(tidyr)
library(dplyr)
tilskudd <- read.csv("data_flat_output/tilskudd-kommune-2011_2014.csv", row.names=NULL, na.strings="NA", stringsAsFactors=FALSE, sep=",", dec=".")
kinfo <- read.csv("parameters/kommuneegenskaper.csv", sep=",", stringsAsFactors=FALSE)
#begge settene får kommunenummer med 4 siffer.
kinfo = distinct(select(kinfo,kommune_nr,naringsregion_nr),kommune_nr)
...
#her blir merge feil, må være inner_join (inner_join muterer og gir duplikater, mens semi_join ikke gir kolonna - håndterer duplikater lenger opp)
df = inner_join(tilskudd,kinfo,by="kommune_nr")
sum(is.na(df))
tilskudd_data = melt.data.frame(df,id.vars=c("kommune_nr","naringsregion_nr","aar","tabell_navn","enhet"),na.rm=F)
tilskudd_nareg = cast(tilskudd_data,naringsregion_nr~variable+aar,fun.aggregate=sum,na.rm=F,add.missing=T,fill=NA)
#kan bruke separate for å skille ut informasjon om år til egen variabel
df = gather(tilskudd_nareg,"tilskudd_til_kommuner","tabellvariabel",barnehage_tilsk_2011:totalt_2014)
df = separate(df,tilskudd_til_kommuner,into=c("tilskudd_til_kommuner","slett","aar"),sep=c(-6,-5))
df = select(df,-slett)
#resterende
df$tabell_navn="tilskudd"
df$enhet="kroner"
#her er næringsregion blitt satt til factor
tilskudd_nareg$naringsregion_nr = as.numeric(levels(tilskudd_nareg$naringsregion_nr))[tilskudd_nareg$naringsregion_nr]
df$naringsregion_nr[nchar(df$naringsregion_nr)==1] = paste0("0",df$naringsregion_nr[nchar(df$naringsregion_nr)==1])
#logisk og substansiell sjekk
checksum_start == sum(df$tabellvariabel)
apply(subset(df,select=-tabellvariabel),2, function(x){levels(as.factor(x))})
nrow(df)==84*10*4
#utskrift
write.csv(df,"data_flat_output/tilskudd-naringsregion-2011_2014.csv",row.names=F)
...
tilskudd <- read.csv("data_flat_output/tilskudd-kommune-2011_2014.csv", row.names=NULL, na.strings="NA", stringsAsFactors=FALSE, sep=",", dec=".")
kinfo <- read.csv("parameters/kommuneegenskaper.csv", sep=",", stringsAsFactors=FALSE)
#begge settene får kommunenummer med 3 siffer. så lenge det er identisk går det fint
kinfo = distinct(select(kinfo,kommune_nr,fylke_nr),kommune_nr)
checksum_start = sum(tilskudd$tabellvariabel)
#input for aggregering bør være krysstabellen, ikke flatfila.
tilskudd = spread(tilskudd,tilskudd_til_kommuner,tabellvariabel)
#her blir merge feil, må være inner_join (inner_join muterer og gir duplikater, mens semi_join ikke gir kolonna - håndterer duplikater lenger opp)
...
tilskudd_fylke = cast(tilskudd_data,fylke_nr~variable+aar,fun.aggregate=sum,na.rm=F,add.missing=T,fill=NA,margins="grand_row")
#fjerne overflødige kombinasjoner
tilskudd_fylke = select(tilskudd_fylke,-grep("_(all)",names(tilskudd_fylke),fixed=T),-grep("(all)_",names(tilskudd_fylke),fixed=T))
tilskudd_fylke$fylke_nr = levels(tilskudd_fylke$fylke_nr)[tilskudd_fylke$fylke_nr]
tilskudd_fylke$fylke_nr = gsub(".all.","00",tilskudd_fylke$fylke_nr)
#kan bruke separate for å skille ut informasjon om år til egen variabel
df = gather(tilskudd_fylke,"tilskudd_til_kommuner","tabellvariabel",barnehage_tilsk_2011:totalt_2014)
df = separate(df,tilskudd_til_kommuner,into=c("tilskudd_til_kommuner","slett","aar"),sep=c(-6,-5))
df = select(df,-slett)
#resterende
df$tabell_navn="tilskudd"
...
df$fylke_nr[nchar(df$fylke_nr)==1] = paste0("0",df$fylke_nr[nchar(df$fylke_nr)==1])
#logisk og substansiell sjekk
checksum_start*2 == sum(df$tabellvariabel) #*2 pga 00-summering
apply(subset(df,select=-tabellvariabel),2, function(x){levels(as.factor(x))})
nrow(df)==20*10*4
#utskrift
write.csv(df,"data_flat_output/tilskudd-fylke-2011_2014.csv",row.names=F)
